...
Low and normal toasts stay on screen for at least 5s and 8s respectively, and use the sender's `expire_timeout` up to a 30s maximum. Critical toasts stay until dismissed unless the sender requests a positive timeout, which is also capped at 30s. Hovering pauses the countdown, and a content update restarts it — new text deserves a full look. Left-click invokes the default action, right-click or the hover-revealed close button dismisses.
